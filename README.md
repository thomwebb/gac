...
- **MCP server**: Run `uvx gac serve` to expose commit tools to AI agents via the [Model Context Protocol](https://modelcontextprotocol.io/)

### 📊 **Usage Statistics**

```bash
uvx gac stats               # Overview: total gacs, streaks, daily/weekly peaks, top projects & models
uvx gac stats models        # Per-model breakdown: gacs, tokens, latency, speed, per-commit latency charts
uvx gac stats projects      # Per-project breakdown: gacs, commits, tokens, avg files per gac across all repos
uvx gac stats recent        # Recent gac history: per-gac tokens, speed, latency, files (-n 20 for more)
...
| `uvx gac model`     | Change AI provider and model                                            |
...
See `.gac.env.example` for all available options, including `GAC_ALWAYS_GROUPED` to always use grouped commit mode, `GAC_ALWAYS_INCLUDE_SCOPE` to auto-infer commit scopes, and many others for customization.
...
- **Usage statistics**: See `uvx gac stats --help` or the [full docs](docs/en/USAGE.md#usage-statistics)
