...
def test_configure_model_lilac_success(tmp_path):
    """Test successful Lilac provider configuration."""
    env_path = tmp_path / ".gac.env"
    env_path.touch()
    ...
    with patch("gac.model_cli.GAC_ENV_PATH", env_path):
        with (
            ...
            # Select Lilac provider
            mselect.return_value.ask.return_value = "Lilac"
            mtext.return_value.ask.return_value = "google/gemma-4-31b-it"
            mpass.return_value.ask.return_value = "lilac-key"
...
def test_configure_model_deepinfra_success(tmp_path):
    ...
    with patch("gac.model_cli.GAC_ENV_PATH", env_path):
        with (
            ...
            # Select DeepInfra provider
            mselect.return_value.ask.return_value = "DeepInfra"
            mtext.return_value.ask.return_value = "meta-llama/Llama-3.1-70B-Instruct"
            mpass.return_value.ask.return_value = "deepinfra-key"
