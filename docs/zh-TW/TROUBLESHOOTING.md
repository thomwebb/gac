...
**重要：**密鑰掃描在**任何 AI API 呼叫之前**執行。如果檢測到密鑰，工作流程將立即中止，不會進行任何 API 呼叫。掃描程式使用**基於正則表達式的模式匹配**（而非 LLM），因此掃描速度快且完全在本機執行 — 您的程式碼永遠不會傳送到 AI 模型進行密鑰檢測。

**問題：** 誤報：密鑰掃描檢測到非密鑰

- 安全掃描程式會尋找類似於 API 密鑰、權杖和密碼的正則表達式模式
...
- 掃描程式使用基於正則表達式的模式匹配（而非 LLM），可能無法捕獲所有密鑰類型
