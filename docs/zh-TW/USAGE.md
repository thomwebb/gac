...
uvx gac 包含內建的安全掃描，在提交之前自動檢測暫存變更中的潛在密鑰和 API 金鑰。這有助於防止意外提交敏感資訊。
...
- `uvx gac stats show` — 檢視你的 gac 使用統計（總計、連續使用、每日和每週活動、令牌使用量、熱門專案、熱門模型）
- `uvx gac stats models` — 檢視所有模型的詳細統計，包含令牌明細和速度對比圖表
- `uvx gac stats projects` — 檢視所有專案的統計，包含令牌明細
...
| `uvx gac stats models`                 | 顯示**所有**已使用模型的詳細統計，包含令牌明細和速度對比圖表                 |
| `uvx gac stats projects`               | 顯示**所有**專案的統計，包含令牌明細                                         |
| `uvx gac stats reset`                  | 重置所有統計資料為零（需要確認）                                             |
| `uvx gac stats reset model <model-id>` | 重置特定模型的統計資料（不區分大小寫）                                       |
...
Running `uvx gac stats projects` 顯示**所有**專案（不僅僅是前 5 個），包含：

- **所有專案表格** — 每個專案按活動排序，包含 gac 次數、提交次數、提示令牌、完成令牌、推理令牌和總令牌
- **熱門模型** — 5 個最常用模型及其提示、完成和總令牌消耗量

Running `uvx gac stats models` 顯示**所有**模型（不僅僅是前 5 個），包含：
