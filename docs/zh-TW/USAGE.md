...
本文件介紹了 `uvx gac` CLI 工具的所有可用標誌和選項。
...
- 組合標誌以獲得更強大的工作流程（例如，`uvx gac -ayp` 以暫存、自動確認和推送）
...
```sh
uvx gac --no-verify  # 跳過所有 pre-commit 和 lefthook 鉤子
```
...
uvx gac 包含內建的安全掃描，在**任何 AI API 呼叫之前**自動檢測暫存變更中的潛在密鑰和 API 金鑰。如果檢測到密鑰，工作流程將立即中止 — 不會進行任何 API 呼叫。這確保了您的敏感資料永遠不會傳送到任何 AI 模型。掃描程式使用**基於正則表達式的模式匹配**，而非 LLM，因此掃描速度快且完全在本機執行。

**跳過安全掃描：**

```sh
uvx gac --skip-secret-scan  # 為此次提交跳過安全掃描
```
...
```sh
uvx gac --no-verify-ssl  # 跳過此次提交的 SSL 驗證
```
...
uvx gac 支援向提交訊息新增 `Signed-off-by` 行，這在許多開源專案中是 [Developer Certificate of Origin (DCO)](https://developercertificate.org/) 合規所必需的。

**新增 signoff :**

```sh
uvx gac --signoff  # 新增 Signed-off-by 行到提交訊息（DCO 合規）
```
...
- 設定 gac 的推薦方法是執行 `uvx gac init` 並按照互動式提示操作。
- 已經設定好語言，只想切換提供者或模型？執行 `uvx gac model`，它會跳過所有語言相關的問題。
...
- `GAC_REASONING_EFFORT=medium` - 控制支援延伸思考模型之推理/思考深度（low、medium、high、max）。勿設定則使用各模型預設值。僅傳送至相容之提供者（OpenAI 風格；非 Anthropic）。
...
- `uvx gac init` — 提供者、模型和語言設定的互動式設定精靈
- `uvx gac model` — 提供者/模型/API 金鑰設定，無語言提示（適合快速切換）
- `uvx gac auth` — 顯示所有提供者的 OAuth 驗證狀態
- `uvx gac auth claude-code login` — 使用 OAuth 登入 Claude Code（開啟瀏覽器）
- `uvx gac auth claude-code logout` — 從 Claude Code 登出並刪除儲存的權杖
- `uvx gac auth claude-code status` — 檢查 Claude Code 身份驗證狀態
- `uvx gac auth chatgpt login` — 使用 OAuth 登入 ChatGPT（開啟瀏覽器）
- `uvx gac auth chatgpt logout` — 從 ChatGPT 登出並刪除儲存的權杖
- `uvx gac auth chatgpt status` — 檢查 ChatGPT 身份驗證狀態
- `uvx gac auth copilot login` — 使用 Device Flow 登入 GitHub Copilot
- `uvx gac auth copilot login --host ghe.mycompany.com` — 登入 GitHub Enterprise 實例上的 Copilot
- `uvx gac auth copilot logout` — 從 Copilot 登出並刪除儲存的令牌
- `uvx gac auth copilot status` — 檢查 Copilot 身份驗證狀態
- `uvx gac config show`
- `uvx gac config set KEY VALUE` — 在 `$HOME/.gac.env` 中設定設定金鑰
- `uvx gac config get KEY` — 獲取設定值
- `uvx gac config unset KEY` — 從 `$HOME/.gac.env` 中刪除設定金鑰
- `uvx gac language`（或 `uvx gac lang`）— 提交訊息的互動式語言選擇器（設定 GAC_LANGUAGE）
- `uvx gac editor`（或 `uvx gac edit`）— 確認提示符中 `e` 鍵的互動式編輯器選擇器（設定 GAC_EDITOR）
- `uvx gac diff` — 顯示過濾的 git diff，具有暫存/未暫存變更、顏色和截斷選項
- `uvx gac serve` — 將 GAC 作為 [MCP 伺服器](MCP.md) 啟動，用於 AI 代理整合（stdio 傳輸）
- `uvx gac stats show` — 檢視你的 gac 使用統計（總計、連續使用、每日和每週活動、令牌使用量、帶平均檔案數的熱門專案、帶速度和延遲的熱門模型）
- `uvx gac stats models` — 所有模型的詳細統計，包含令牌細分、速度、延遲和每次提交延遲對比圖表
- `uvx gac stats projects` — 所有專案的統計，包含令牌細分和每次 gac 平均檔案數
- `uvx gac stats recent` — 最近 10 次 gac 的歷史記錄，包含令牌、速度、延遲、檔案數（`-n 20` 查看更多）
- `uvx gac stats reset` — 重置所有統計資料為零（需要確認）
- `uvx gac stats reset model <model-id>` — 重置特定模型的統計資料（不區分大小寫）
...
uvx gac 追蹤輕量級使用統計，讓你可以檢視提交活動、連續使用天數、令牌使用量和最活躍的專案及模型。統計資料本地儲存在 `~/.gac_stats.json` 中，不會傳送到任何地方 — 無遙測。
...
`uvx gac init` 會詢問是否啟用統計，並詳細說明儲存的內容。你可以隨時改變主意：

- **啟用統計：** 取消設定 `GAC_DISABLE_STATS` 或設定為 `false`/`0`/`no`/`off`/空。
- **停用統計：** 將 `GAC_DISABLE_STATS` 設定為 truthy 值（`true`、`1`、`yes`、`on`）。

在 `uvx gac init` 中拒絕統計時，如果偵測到現有的 `~/.gac_stats.json`，將提供刪除選項。

### 統計子命令

| 命令                                   | 描述                                                                         |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `uvx gac stats`                        | 顯示你的統計（等同於 `uvx gac stats show`）                                  |
| `uvx gac stats show`                   | 顯示完整統計：總計、連續使用、每日和每週活動、令牌使用量、熱門專案、熱門模型 |
| `uvx gac stats models`                 | 所有使用模型的詳細統計，包含令牌細分、速度、延遲和每次提交延遲圖表           |
| `uvx gac stats projects`               | 所有專案的統計，包含令牌細分和每次 gac 平均檔案數                            |
| `uvx gac stats recent`                 | 最近 10 次 gac（`-n 20` 查看更多），包含令牌、速度、延遲、檔案數             |
| `uvx gac stats reset`                  | 重置所有統計資料為零（需要確認）                                             |
| `uvx gac stats reset model <model-id>` | 重置特定模型的統計資料（不區分大小寫）                                       |
...
執行 `uvx gac stats` 將顯示：

- **gac 使用總次數和提交總次數** — 你使用 gac 的次數和它建立的提交數
- **當前和最長連續使用天數** — 連續使用 gac 的天數（5 天及以上顯示 🔥）
- **活動摘要** — 今日和本週的 gac 使用、提交和令牌數與你的峰值日和峰值週對比
- **熱門專案** — 按 gac 使用 + 提交數排列的 5 個最活躍倉庫，包含每次 gac 平均檔案數和令牌使用量
- **熱門模型** — 5 個最常用模型及其全時段速度、延遲和令牌使用量

執行 `uvx gac stats projects` 顯示**所有**專案（不僅僅是前 5 個），包含：

- **所有專案表格** — 每個專案按活動排序，包含 gac 次數、提交次數、每次 gac 提交比率、每次 gac 平均檔案數、提示令牌、完成令牌、推理令牌、總令牌和總 gac 佔比
- **活動條形圖** — 水平條形顯示每個專案的相對 gac 次數
- **令牌使用量條形圖** — 水平條形顯示每個專案的相對令牌消耗量

執行 `uvx gac stats models` 顯示**所有**模型（不僅僅是前 5 個），包含：

- **所有模型表格** — 每個已使用的模型按活動排序，包含 gac 次數、提交次數、全時段速度（令牌/秒）、全時段延遲、提示令牌、完成令牌、推理令牌和總令牌
- **速度對比（30天）圖表** — 最近（過去 30 天）模型速度的水平條形圖，從最快到最慢排序，按速度百分位著色（🟡 極速、🟢 快速、🔵 中等、🔘 慢速）
- **延遲對比（30天）圖表** — 最近每次呼叫延遲的水平條形圖，從最短到最長排序
- **每次提交延遲（30天）圖表** — 最近延遲除以提交次數的水平條形圖，顯示每次提交的實際等待時間（在 10 秒 gac 中完成 5 次提交的模型每次提交耗時 2 秒 vs 在 25 秒 gac 中完成 1 次提交的模型每次提交耗時 25 秒）
- **高分慶祝** — 🏆 在你創造新的每日、每週、令牌或連續使用紀錄時獲得獎盃；🥈 在追平紀錄時獲得
- **鼓勵訊息** — 基於你活動的上下文鼓勵

執行 `uvx gac stats recent` 顯示你最近 10 次 gac（可透過 `-n` 設定）：

- **最近 gac 表格** — 每次 gac 包含相對時間、專案、模型、提交次數、檔案數、速度、延遲和每次 gac 的令牌明細
...
gac 可以在每次提交時透過您頻道整合設定中的 Webhook URL 向 Discord 頻道發送通知。此整合是**選用的**：在您明確設定 Webhook URL 之前，它不會執行任何動作。
...
```bash
uvx gac discord setup     # 以互動方式設定 Webhook URL
uvx gac discord show      # 顯示是否已設定 Webhook（URL 已遮蔽）
uvx gac discord test      # 向已設定的 Webhook 發送測試通知
uvx gac discord remove    # 刪除已設定的 Webhook URL
```

或者，直接在 `$HOME/.gac.env`（或 `./.gac.env`）中設定變數：
...
- 在每次成功提交後觸發（單個和分組的工作流）。在 `--dry-run` 和 `--message-only` 時跳過。
- 發布帶有綠色條紋的 GitHub 風格 **嵌入**，倉庫 + 分支作為作者行，提交主題作為標題，提交正文作為描述，短 SHA 在頁腳中。
- 使用 gac 頭像和使用者名稱 `gac`。
- Webhook 失敗會以 WARNING 級別記錄，並且**從不**阻擋您的提交。
- 將 `GAC_DISCORD_WEBHOOK_URL` 保持未設定（或為空）以停用。`gac init` 不受影響 — Discord 設定僅位於 `gac discord` 下。
