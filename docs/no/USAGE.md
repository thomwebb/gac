...
- `GAC_REASONING_EFFORT=medium` - Kontroller resonnerings-/tankedybde for modeller som støtter utvidet tenkning (low, medium, high, max). La være uinnstilt for å bruke hver modells standard. Sendes kun til kompatible leverandører (OpenAI-stil; ikke Anthropic).
