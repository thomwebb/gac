...
- `uvx gac stats show` — gac 사용 통계 보기 (총계, 연속 사용, 일일 및 주간 활동, 토큰 사용량, 상위 프로젝트, 상위 모델)
- `uvx gac stats models` — 모든 모델의 토큰 분석 및 속도 비교 차트가 포함된 상세 통계 보기
- `uvx gac stats projects` — 모든 프로젝트의 토큰 분석 통계 보기
...
| `uvx gac stats models`                 | 사용한 **모든** 모델의 토큰 분석 및 속도 비교 차트가 포함된 상세 통계 표시                |
| `uvx gac stats projects`               | **모든** 프로젝트의 토큰 분석 통계 표시                                                   |
| `uvx gac stats reset`                  | 모든 통계를 0으로 재설정 (확인 프롬프트 표시)                                             |
| `uvx gac stats reset model <model-id>` | 특정 모델의 통계 재설정 (대소문자 구분 없음)                                              |
...
Running `uvx gac stats projects`은 (상위 5개뿐만 아니라) **모든** 프로젝트를 다음과 함께 표시합니다:

- **전체 프로젝트 표** — 활동별로 정렬된 각 프로젝트, gac 횟수, 커밋 횟수, 프롬프트 토큰, 완료 토큰, 추론 토큰 및 총 토큰 포함
- **상위 모델** — 프롬프트, 완료, 총 토큰 소비량이 포함된 가장 많이 사용된 상위 5개 모델

Running `uvx gac stats models`은 (상위 5개뿐만 아니라) **모든** 모델을 다음과 함께 표시합니다:
