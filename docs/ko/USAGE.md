...
이 문서는 `uvx gac` CLI 도구의 사용 가능한 모든 플래그와 옵션을 설명합니다.
...
- 더 강력한 워크플로우를 위해 플래그 결합 (예: `uvx gac -ayp`로 스테이징, 자동 확인 및 푸시)
...
```sh
uvx gac --no-verify  # 모든 pre-commit 및 lefthook hooks 건너뛰기
```
...
gac에는 **어떤 AI API 호출이 이루어지기 전에** 스테이징된 변경 사항에서 잠재적인 비밀과 API 키를 자동으로 감지하는 내장 보안 검사가 포함되어 있습니다. 비밀이 감지되면 워크플로우가 즉시 중단됩니다 — API 호출이 발생하지 않습니다. 이는 민감한 데이터가 AI 모델로 전송되지 않도록 보장합니다. 스캐너는 **정규식 기반 패턴 매칭**을 사용하며 LLM이 아니므로 스캔이 빠르고 완전히 로컬에서 실행됩니다.

**보안 검사 건너뛰기:**

```sh
uvx gac --skip-secret-scan  # 이 커밋에 대한 보안 검사 건너뛰기
```
...
```sh
uvx gac --no-verify-ssl  # 이 커밋에 대한 SSL 검증 건너뛰기
```
...
```sh
uvx gac --signoff  # 커밋 메시지에 Signed-off-by 라인 추가 (DCO 규정 준수)
```
...
- gac를 설정하는 권장 방법은 `uvx gac init`를 실행하고 대화형 프롬프트를 따르는 것입니다.
- 이미 언어가 구성되었고 프로바이더나 모델만 전환해야 하나요? 언어 질문 없이 설정을 반복하려면 `uvx gac model`을 실행하세요.
...
- `GAC_REASONING_EFFORT=medium` - 확장된 사고를 지원하는 모델의 추론/사고 깊이 제어 (low, medium, high, max). 설정하지 않으면 각 모델의 기본값을 사용합니다. 호환되는 공급자(OpenAI 스타일, Anthropic 제외)에게만 전송됩니다.
...
- `uvx gac init` — 프로바이더, 모델, 언어 구성을 위한 대화형 설정 마법사
- `uvx gac model` — 언어 프롬프트 없는 프로바이더/모델/API 키 설정 (빠른 전환에 이상적)
- `uvx gac auth` — 모든 프로바이더의 OAuth 인증 상태 표시
- `uvx gac auth claude-code login` — OAuth를 사용하여 Claude Code에 로그인 (브라우저 열림)
- `uvx gac auth claude-code logout` — Claude Code에서 로그아웃하고 저장된 토큰 제거
- `uvx gac auth claude-code status` — Claude Code 인증 상태 확인
- `uvx gac auth chatgpt login` — OAuth 를 사용하여 ChatGPT 에 로그인 (브라우저 열림)
- `uvx gac auth chatgpt logout` — ChatGPT 에서 로그아웃하고 저장된 토큰 제거
- `uvx gac auth chatgpt status` — ChatGPT 인증 상태 확인
- `uvx gac auth copilot login` — Device Flow를 사용하여 GitHub Copilot에 로그인
- `uvx gac auth copilot login --host ghe.mycompany.com` — GitHub Enterprise 인스턴스의 Copilot에 로그인
- `uvx gac auth copilot logout` — Copilot에서 로그아웃하고 저장된 토큰 삭제
- `uvx gac auth copilot status` — Copilot 인증 상태 확인
- `uvx gac config show` — 현재 구성 표시
- `uvx gac config set KEY VALUE` — `$HOME/.gac.env`에서 구성 키 설정
- `uvx gac config get KEY` — 구성 값 가져오기
- `uvx gac config unset KEY` — `$HOME/.gac.env`에서 구성 키 제거
- `uvx gac language` (또는 `uvx gac lang`) — 커밋 메시지를 위한 대화형 언어 선택기 (GAC_LANGUAGE 설정)
- `uvx gac editor` (또는 `uvx gac edit`) — 확인 프롬프트에서 `e` 키를 위한 대화형 에디터 선택기 (GAC_EDITOR 설정)
- `uvx gac diff` — 스테이징된/스테이징되지 않은 변경, 색상, 자르기 옵션으로 필터링된 git diff 표시
- `uvx gac serve` — AI 에이전트 통합을 위한 [MCP 서버](MCP.md)로 GAC 시작 (stdio 전송)
- `uvx gac stats show` — gac 사용 통계 보기 (총계, 연속 사용, 일일 및 주간 활동, 토큰 사용량, 평균 파일 수 포함 상위 프로젝트, 속도 및 지연 시간 포함 상위 모델)
- `uvx gac stats models` — 모든 모델의 상세 통계, 토큰 내역, 속도, 지연 시간 및 커밋당 지연 시간 차트 포함
- `uvx gac stats projects` — 모든 프로젝트의 통계, 토큰 내역 및 gac당 평균 파일 수 포함
- `uvx gac stats recent` — 최근 10개 gac 기록, gac당 토큰, 속도, 지연 시간, 파일 수 (`-n 20`으로 더 보기)
- `uvx gac stats reset` — 모든 통계를 0으로 재설정 (확인 프롬프트 표시)
- `uvx gac stats reset model <model-id>` — 특정 모델의 통계 재설정 (대소문자 구분 없음)
...
```sh
uvx gac -i -h "사용자 프로필을 위한 데이터베이스 마이그레이션"
...
`uvx gac init`이 통계 활성화 여부를 묻고 저장되는 항목을 설명합니다. 언제든 마음을 바꿀 수 있습니다:

- **통계 활성화:** `GAC_DISABLE_STATS`를 해제하거나 `false`/`0`/`no`/`off`/비어 있음으로 설정합니다.
- **통계 비활성화:** `GAC_DISABLE_STATS`를 truthy 값 (`true`, `1`, `yes`, `on`)으로 설정합니다.

`uvx gac init` 중에 통계를 거부하고 기존 `~/.gac_stats.json`이 감지되면 삭제 옵션이 제공됩니다.

### 통계 하위 명령어

| 명령어                                 | 설명                                                                                      |
| -------------------------------------- | ----------------------------------------------------------------------------------------- |
| `uvx gac stats`                        | 통계 표시 (`uvx gac stats show`과 동일)                                                   |
| `uvx gac stats show`                   | 전체 통계 표시: 총계, 연속 사용, 일일 및 주간 활동, 토큰 사용량, 상위 프로젝트, 상위 모델 |
| `uvx gac stats models`                 | **모든** 사용 모델의 상세 통계, 토큰 내역, 속도, 지연 시간 및 커밋당 지연 시간 차트 포함  |
| `uvx gac stats projects`               | **모든** 프로젝트의 통계, 토큰 내역 및 gac당 평균 파일 수 포함                            |
| `uvx gac stats recent`                 | 최근 10개 gac (`-n 20`으로 더 보기), gac당 토큰, 속도, 지연 시간, 파일 수                 |
| `uvx gac stats reset`                  | 모든 통계를 0으로 재설정 (확인 프롬프트 표시)                                             |
| `uvx gac stats reset model <model-id>` | 특정 모델의 통계 재설정 (대소문자 구분 없음)                                              |
...
`uvx gac stats`를 실행하면 다음이 표시됩니다:

- **gac 및 커밋 총계** — gac을 사용한 횟수와 생성된 커밋 수
- **현재 및 최장 연속 일수** — gac 활동이 있는 연속 일수 (5일 이상이면 🔥)
- **활동 요약** — 오늘 및 이번 주 gac, 커밋, 토큰 수와 피크 일 및 피크 주 비교
- **상위 프로젝트** — gac 사용 + 커밋 수 기준 상위 5개 가장 활발한 저장소, gac당 평균 파일 수와 토큰 사용량 포함
- **상위 모델** — 전체 기간 속도, 지연 시간 및 토큰 사용량이 포함된 가장 많이 사용된 상위 5개 모델

`uvx gac stats projects`은 (상위 5개뿐만 아니라) **모든** 프로젝트를 다음과 함께 표시합니다:

- **전체 프로젝트 표** — 활동별로 정렬된 각 프로젝트, gac 횟수, 커밋 횟수, gac당 커밋 비율, gac당 평균 파일 수, 프롬프트 토큰, 완료 토큰, 추론 토큰, 총 토큰 및 전체 gac 대비 비율 포함
- **활동 바 차트** — 프로젝트별 상대적 gac 횟수를 보여주는 수평 바
- **토큰 사용량 바 차트** — 프로젝트별 상대적 토큰 소비량을 보여주는 수평 바

`uvx gac stats models`은 (상위 5개뿐만 아니라) **모든** 모델을 다음과 함께 표시합니다:

- **전체 모델 표** — 활동별로 정렬된 각 사용 모델, gac 횟수, 커밋 횟수, 전체 기간 속도(토큰/초), 전체 기간 지연 시간, 프롬프트 토큰, 완료 토큰, 추론 토큰 및 총 토큰 포함
- **속도 비교 (30일) 차트** — 최근(지난 30일) 모델 속도의 수평 막대 차트, 가장 빠른 것부터 가장 느린 것까지 정렬, 속도 백분위수별 색상 코딩 (🟡 초고속, 🟢 빠름, 🔵 보통, 🔘 느림)
- **지연 시간 비교 (30일) 차트** — 최근 호출당 지연 시간의 수평 막대 차트, 가장 짧은 것부터 가장 긴 것까지 정렬
- **커밋당 지연 시간 (30일) 차트** — 최근 지연 시간을 커밋 수로 나눈 수평 막대 차트, 커밋당 실제 대기 시간 표시 (10초 gac에서 5개 커밋을 하는 모델은 2초/커밋 vs 25초 gac에서 1개 커밋을 하는 모델은 25초/커밋)
- **하이 스코어 축하** — 🏆 새로운 일일, 주간, 토큰 또는 연속 기록을 세우면 트로피 획득; 🥈 기록과 타이면 획득
- **격려 메시지** — 활동에 기반한 맥락적 응원

`uvx gac stats recent`는 최근 10개의 gac을 표시합니다(`-n`으로 설정 가능):

- **최근 gac 표** — 각 gac에 상대적 시간, 프로젝트, 모델, 커밋 수, 파일 수, 속도, 지연 시간 및 gac당 토큰 내역 포함
...
gac는 커밋할 때마다 채널의 통합 설정에서 웹훅 URL을 사용하여 Discord 채널에 알림을 보낼 수 있습니다. 이 통합은 **옵트인**입니다: 웹훅 URL을 명시적으로 구성할 때까지 아무 작업도 수행하지 않습니다.

### 설정

전용 `discord` 하위 명령 그룹을 사용하세요:

```bash
uvx gac discord setup     # 웹훅 URL을 대화식으로 구성
uvx gac discord show      # 웹훅이 구성되어 있는지 표시 (URL 마스킹됨)
uvx gac discord test      # 구성된 웹훅에 테스트 알림 전송
uvx gac discord remove    # 구성된 웹훅 URL 제거
```

또는 `$HOME/.gac.env`（또는`./.gac.env`）에 변수를 직접 설정하세요:
...
- 각 성공적인 커밋 후에 발생합니다（단일 및 그룹화된 워크플로）。`--dry-run`과`--message-only`에서는 건너뜁니다。
- 녹색 스트라이프가 있는 GitHub 스타일 **임베드**를 게시합니다: 작성자 행으로 레포+브랜치, 제목으로 커밋 제목, 설명으로 커밋 본문, 바닥글에 짧은 SHA를 포함합니다.
- gac 아바타와 사용자 이름 `gac`를 사용합니다.
- 웹훅 실패는 WARNING으로 기록되며 커밋을 **절대** 차단하지 않습니다.
- 비활성화하려면 `GAC_DISCORD_WEBHOOK_URL`을 설정하지 않거나 비워두세요. `gac init`은 영향을 받지 않습니다 — Discord 설정은 `gac discord`에만 있습니다.
