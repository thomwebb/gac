...
**अपनी कमिट्स को स्वचालित करें!** `git commit -m "..."` को `uvx gac` से बदलें संदर्भात्मक, अच्छी तरह से स्वरूपित कमिट मैसेज के लिए जो बड़े भाषा मॉडल द्वारा जेनरेट किए गए हैं!
...
- **25+ भाषाएं**: अंग्रेजी, चीनी, जापानी, कोरियाई, स्पैनिश, फ्रेंच, जर्मन, और 18+ और भाषाओं में कमिट मैसेज जेनरेट करें
...
- **कई वर्कफ्लोज़**: `uvx gac language` के साथ डिफ़ॉल्ट भाषा सेट करें, या एक बार के ओवरराइड के लिए `-l <language>` फ्लैग का उपयोग करें
...
- **वन-कमांड वर्कफ्लोज़**: `uvx gac -ayp` (सभी को स्टेज करें, ऑटो-कन्फर्म करें, पुश करें) जैसे फ्लैग्स के साथ पूर्ण वर्कफ्लोज़
- **Git इंटीग्रेशन**: प्री-कमिट और लेफ्थुक हुक्स का सम्मान करता है, महंगे LLM ऑपरेशन से पहले उन्हें चलाता है
- **MCP सर्वर**: [Model Context Protocol](https://modelcontextprotocol.io/) के माध्यम से AI एजेंटों को कमिट टूल्स उपलब्ध कराने के लिए `uvx gac serve` चलाएं
...
- **सेटअप में ऑप्ट-इन**: `uvx gac init` पूछता है कि क्या आप आँकड़े सक्षम करना चाहते हैं और स्पष्ट रूप से बताता है कि क्या संग्रहीत किया जाता है
...
| Command             | Description                                                         |
| ------------------- | ------------------------------------------------------------------- |
| `uvx gac`           | कमिट मैसेज जेनरेट करें                                              |
| `uvx gac -y`        | ऑटो-कन्फर्म (समीक्षा की आवश्यकता नहीं)                              |
| `uvx gac -a`        | कमिट मैसेज जेनरेट करने से पहले सभी को स्टेज करें                    |
| `uvx gac -S`        | इंटरैक्टिव रूप से स्टेज करने के लिए फाइलें चुनें                    |
| `uvx gac -o`        | तुच्छ परिवर्तनों के लिए वन-लाइन मैसेज                               |
| `uvx gac -v`        | प्रेरणा, तकनीकी दृष्टिकोण, और प्रभाव विश्लेषण के साथ वर्बोज फॉर्मेट |
| `uvx gac -h "hint"` | LLM के लिए संदर्भ जोड़ें (जैसे, `uvx gac -h "bug fix"`)             |
| `uvx gac -s`        | स्कोप शामिल करें (जैसे, feat(auth):)                                |
| `uvx gac -i`        | परिवर्तनों के बारे में प्रश्न पूछें बेहतर संदर्भ के लिए             |
| `uvx gac -g`        | परिवर्तनों को कई तार्किक कमिट्स में समूहित करें                     |
| `uvx gac -p`        | कमिट और पुश करें                                                    |
| `uvx gac model`     | AI प्रदाता और मॉडल बदलें                                            |
| `uvx gac stats`     | अपनी gac उपयोग सांख्यिकी देखें                                      |
...
अपने प्रोवाइडर को इंटरैक्टिव रूप से कॉन्फिगर करने के लिए `uvx gac init` चलाएं, या एनवायरमेंट वेरिएबल्स सेट करें:

बाद में प्रोवाइडर या मॉडल बदलने की जरूरत है भाषा सेटिंग्स को छुए बिना? भाषा प्रॉम्प्ट को छोड़ने वाली स्ट्रीमलाइन फ्लो के लिए `uvx gac model` का उपयोग करें।

सभी उपलब्ध विकल्पों के लिए `.gac.env.example` देखें।

**दूसरी भाषा में कमिट मैसेज चाहिए?** Español, Français, 日本語, और अन्य सहित 25+ भाषाओं में से चुनने के लिए `uvx gac language` चलाएं।
...
- **उपयोग सांख्यिकी**: `uvx gac stats --help` देखें या [पूरे डॉक्स](docs/hi/USAGE.md#उपयोग-सांख्यिकी)
