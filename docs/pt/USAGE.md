...
Este documento descreve todas as flags e opções disponíveis para a ferramenta CLI `uvx gac`.
...
- Combine flags para fluxos de trabalho mais poderosos (ex: `uvx gac -ayp` para fazer stage, auto-confirmar e push)
...
```sh
uvx gac --no-verify  # Ignorar todos os hooks pre-commit e lefthook
```
...
```sh
uvx gac --skip-secret-scan  # Ignorar varredura de segurança para este commit
```
...
```sh
uvx gac --no-verify-ssl  # Ignorar verificação SSL para este commit
```
...
uvx gac suporta adicionar uma linha `Signed-off-by` às mensagens de commit, que é necessária para conformidade com o [Developer Certificate of Origin (DCO)](https://developercertificate.org/) em muitos projetos de código aberto.

**Adicionar signoff :**

```sh
uvx gac --signoff  # Adicionar linha Signed-off-by à mensagem de commit (conformidade DCO)
```
...
- A maneira recomendada de configurar o gac é executar `uvx gac init` e seguir os prompts interativos.
- Já configurou o idioma e só precisa mudar de provider ou modelo? Execute `uvx gac model` para repetir a configuração sem perguntas de idioma.
...
- gac carrega configuração na seguinte ordem de precedência:
  1. Flags da CLI
  2. Nível de projeto `.gac.env`
  3. Nível de usuário `~/.gac.env`
...
- `GAC_ALWAYS_GROUPED=true` - Usar sempre o modo de commits agrupados (equivalente a sempre passar o sinalizador `-g` ou `--group`)
...
- `GAC_REASONING_EFFORT=medium` - Controlar a profundidade de raciocínio/pensamento para modelos que suportam pensamento estendido (low, medium, high, max). Deixar não definido para usar o padrão de cada modelo. Enviado apenas para provedores compatíveis (estilo OpenAI; não Anthropic).
...
- `uvx gac init` — Assistente de configuração interativo para provedor, modelo e idioma
- `uvx gac model` — Configuração de provedor/modelo/chave API sem prompts de idioma (ideal para mudanças rápidas)
- `uvx gac auth` — Mostrar status de autenticação OAuth para todos os provedores
- `uvx gac auth claude-code login` — Login no Claude Code usando OAuth (abre navegador)
- `uvx gac auth claude-code logout` — Logout do Claude Code e remover token armazenado
- `uvx gac auth claude-code status` — Verificar status de autenticação do Claude Code
- `uvx gac auth chatgpt login` — Login no ChatGPT usando OAuth (abre navegador)
- `uvx gac auth chatgpt logout` — Logout do ChatGPT e remover token armazenado
- `uvx gac auth chatgpt status` — Verificar status de autenticação do ChatGPT
- `uvx gac auth copilot login` — Login no GitHub Copilot usando Device Flow
- `uvx gac auth copilot login --host ghe.mycompany.com` — Login no Copilot em uma instância do GitHub Enterprise
- `uvx gac auth copilot logout` — Logout do Copilot e remover tokens armazenados
- `uvx gac auth copilot status` — Verificar status de autenticação do Copilot
- `uvx gac config show` — Mostrar configuração atual
- `uvx gac config set KEY VALUE` — Definir chave de configuração em `$HOME/.gac.env`
- `uvx gac config get KEY` — Obter valor de configuração
- `uvx gac config unset KEY` — Remover chave de configuração de `$HOME/.gac.env`
- `uvx gac language` (ou `uvx gac lang`) — Seletor de idioma interativo para mensagens de commit (define GAC_LANGUAGE)
- `uvx gac editor` (ou `uvx gac edit`) — Seletor interativo de editor para a tecla `e` no prompt de confirmação (configura GAC_EDITOR)
- `uvx gac diff` — Mostrar git diff filtrado com opções para mudanças preparadas/não preparadas, cor e truncamento
- `uvx gac serve` — Iniciar o GAC como [servidor MCP](MCP.md) para integração com agentes de IA (transporte stdio)
- `uvx gac stats show` — Ver suas estatísticas de uso do gac (totais, sequências, atividade diária e semanal, uso de tokens, projetos principais com arquivos médios, modelos principais com velocidade e latência)
- `uvx gac stats models` — Estatísticas detalhadas para todos os modelos com detalhamento de tokens, velocidade, latência e gráficos latência por commit
- `uvx gac stats projects` — Estatísticas para todos os projetos com detalhamento de tokens e arquivos médios por gac
- `uvx gac stats recent` — Últimos 10 gacs com tokens, velocidade, latência e arquivos por gac (`-n 20` para mais)
- `uvx gac stats reset` — Redefinir todas as estatísticas para zero (solicita confirmação)
- `uvx gac stats reset model <model-id>` — Redefinir estatísticas de um modelo específico (insensível a maiúsculas/minúsculas)
...
```sh
uvx gac -i -h "Migração de banco de dados para perfis de usuário"
...
uvx gac rastreia estatísticas de uso leves para que você possa ver sua atividade de commits, sequências, uso de tokens, e projetos e modelos mais ativos. As estatísticas são armazenadas localmente em `~/.gac_stats.json` e nunca são enviadas para nenhum lugar — não há telemetria.
...
`uvx gac init` pergunta se você deseja ativar as estatísticas e explica exatamente o que é armazenado. Você pode mudar de ideia a qualquer momento:
...
Quando você recusa as estatísticas durante `uvx gac init` e um `~/.gac_stats.json` existente é detectado, será oferecida a opção de excluí-lo.

### Subcomandos de Estatísticas

| Comando                                | Descrição                                                                                                                             |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `uvx gac stats`                        | Mostrar suas estatísticas (o mesmo que `uvx gac stats show`)                                                                          |
| `uvx gac stats show`                   | Exibir estatísticas completas: totais, sequências, atividade diária e semanal, uso de tokens, projetos principais, modelos principais |
| `uvx gac stats models`                 | Estatísticas detalhadas para **todos** os modelos com detalhamento de tokens, velocidade, latência e gráficos latência por commit     |
| `uvx gac stats projects`               | Estatísticas para **todos** os projetos com detalhamento de tokens e arquivos médios por gac                                          |
| `uvx gac stats recent`                 | Últimos 10 gacs (`-n 20` para mais), com tokens, velocidade, latência e arquivos por gac                                              |
| `uvx gac stats reset`                  | Redefinir todas as estatísticas para zero (solicita confirmação)                                                                      |
| `uvx gac stats reset model <model-id>` | Redefinir estatísticas de um modelo específico (insensível a maiúsculas/minúsculas)                                                   |
...
- **Projetos principais** — seus 5 repositórios mais ativos por contagem de gac + commits, com arquivos médios por gac e uso de tokens
- **Modelos principais** — seus 5 modelos mais usados com velocidade histórica, latência e uso de tokens

Executar `uvx gac stats projects` mostra **todos** os projetos (não apenas os 5 principais) com:

- **Tabela de todos os projetos** — cada projeto ordenado por atividade, com contagem de gac, contagem de commits, ratio de commits por gac, arquivos médios por gac, tokens de prompt, tokens de output, tokens de raciocínio, tokens totais e parcela do total de gacs
- **Gráfico de barras de atividade** — barras horizontais mostrando a contagem relativa de gacs por projeto
- **Gráfico de barras de uso de tokens** — barras horizontais mostrando o consumo relativo de tokens por projeto

Executar `uvx gac stats models` mostra **todos** os modelos (não apenas os 5 principais) com:

- **Tabela de todos os modelos** — cada modelo usado ordenado por atividade, com contagem de gac, contagem de commits, velocidade histórica (tokens/seg), latência histórica, tokens de prompt, tokens de output, tokens de raciocínio e tokens totais
- **Gráfico de comparação de velocidade (30d)** — um gráfico de barras horizontais de velocidades recentes (últimos 30 dias) dos modelos, ordenados do mais rápido ao mais lento, coloridos por percentil de velocidade (🟡 ultrarrápido, 🟢 rápido, 🔵 moderado, 🔘 lento)
- **Gráfico de comparação de latência (30d)** — um gráfico de barras horizontais de latência recente por chamada, ordenada da mais curta à mais longa
- **Gráfico de latência por commit (30d)** — um gráfico de barras horizontais de latência recente dividida por contagem de commits, mostrando tempo real de espera por commit (um modelo que faz 5 commits em um gac de 10s custa 2s/commit vs um que faz 1 commit em um gac de 25s a 25s/commit)
...
Executar `uvx gac stats recent` mostra seus últimos 10 gacs (configurável com `-n`):

- **Tabela de gacs recentes** — cada gac com tempo relativo, projeto, modelo, contagem de commits, arquivos, velocidade, latência e detalhamento de tokens por gac
...
## Notificações de Webhook do Discord

gac pode notificar um canal do Discord toda vez que você fizer um commit, usando uma URL de webhook nas configurações de integração do seu canal. A integração é **opt-in**: não faz nada até que você configure explicitamente uma URL de webhook.

### Configuração

Use o grupo de subcomandos `discord` dedicado:

```bash
uvx gac discord setup     # configurar interativamente uma URL de webhook
uvx gac discord show      # mostrar se um webhook está configurado (URL mascarada)
uvx gac discord test      # enviar uma notificação de teste para o webhook configurado
uvx gac discord remove    # remover a URL do webhook configurado
```

Alternativamente, defina a variável diretamente em `$HOME/.gac.env`（ou`./.gac.env`):
...
- Dispara após cada commit bem-sucedido (fluxos de trabalho individuais e agrupados). Ignorado em `--dry-run` e `--message-only`.
- Posta um **embed** estilo GitHub com uma faixa verde, repo + branch como linha de autor, o assunto do commit como título, o corpo do commit como descrição e o SHA curto no rodapé.
- Usa o avatar do gac e o nome de usuário `gac`.
- Falhas de webhook são registradas em WARNING e **nunca** bloqueiam seu commit.
- Deixe `GAC_DISCORD_WEBHOOK_URL` não definido (ou vazio) para desabilitar. `gac init` não é afetado — a configuração do Discord vive apenas sob `gac discord`.
