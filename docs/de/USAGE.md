...
Dieses Dokument beschreibt alle verfügbaren Flags und Optionen für das `uvx gac` CLI-Werkzeug.
...
- Kombinieren Sie Flags für leistungsfähigere Workflows (z.B. `uvx gac -ayp` zum stagen, automatischen Bestätigen und pushen)
...
```sh
uvx gac --no-verify  # Alle pre-commit und lefthook Hooks überspringen
```
...
uvx gac enthält integriertes Sicherheits-Scanning, das automatisch potenzielle Geheimnisse und API-Schlüssel in Ihren gestageten Änderungen erkennt, **bevor ein AI-API-Aufruf erfolgt**. Wenn ein Geheimnis erkannt wird, wird der Workflow sofort abgebrochen — es erfolgt kein API-Aufruf. Dadurch werden Ihre sensiblen Daten niemals an ein KI-Modell gesendet. Der Scanner verwendet **regex-basiertes Pattern-Matching**, keine LLMs, sodass das Scannen schnell ist und vollständig lokal ausgeführt wird.

**Sicherheits-Scans überspringen:**

```sh
uvx gac --skip-secret-scan  # Sicherheits-Scan für diesen Commit überspringen
```
...
**Hinweis:** Der Scanner verwendet regex-basiertes Pattern-Matching (keine LLMs), um gängige Geheimnisformate zu erkennen. Er wird vor jedem AI-API-Aufruf ausgeführt — wenn ein Geheimnis gefunden wird, erfolgt kein API-Aufruf. Überprüfen Sie immer Ihre gestageten Änderungen vor dem Committen.
...
```sh
uvx gac --no-verify-ssl  # SSL-Überprüfung für diesen Commit überspringen
```
...
uvx gac unterstützt das Hinzufügen einer `Signed-off-by`-Zeile zu Commit-Nachrichten, was für die [Developer Certificate of Origin (DCO)](https://developercertificate.org/)-Konformität in vielen Open-Source-Projekten erforderlich ist.

**Signoff hinzufügen:**

```sh
uvx gac --signoff  # Signed-off-by Zeile zum Commit hinzufügen
```
...
- Die empfohlene Methode zur Einrichtung von gac ist, `uvx gac init` auszuführen und den interaktiven Aufforderungen zu folgen.
- Bereits konfigurierte Sprache und nur Anbieter oder Modelle wechseln müssen? Führen Sie `uvx gac model` aus, um die Einrichtung ohne Sprachfragen zu wiederholen.
...
- `GAC_REASONING_EFFORT=medium` - Steuert die Argumentations-/Denktiefe für Modelle, die erweitertes Denken unterstützen (low, medium, high, max). Nicht setzen, um den Standard des jeweiligen Modells zu verwenden. Wird nur an kompatible Anbieter gesendet (OpenAI-Stil; nicht Anthropic).
...
- `uvx gac init` — Interaktiver Einrichtungs-Assistent für Anbieter, Modell und Sprachkonfiguration
- `uvx gac model` — Anbieter/Modell/API-Schlüssel-Einrichtung ohne Sprachaufforderungen (ideal für schnelle Wechsel)
- `uvx gac auth` — Zeige OAuth-Authentifizierungsstatus für alle Anbieter an
- `uvx gac auth claude-code login` — Anmelden zu Claude Code mit OAuth (öffnet Browser)
- `uvx gac auth claude-code logout` — Abmelden von Claude Code und gespeichertes Token entfernen
- `uvx gac auth claude-code status` — Claude Code-Authentifizierungsstatus prüfen
- `uvx gac auth chatgpt login` — Anmelden zu ChatGPT mit OAuth (öffnet Browser)
- `uvx gac auth chatgpt logout` — Abmelden von ChatGPT und gespeichertes Token entfernen
- `uvx gac auth chatgpt status` — ChatGPT-Authentifizierungsstatus prüfen
- `uvx gac auth copilot login` — Login zu GitHub Copilot mit Device Flow
- `uvx gac auth copilot login --host ghe.mycompany.com` — Login zu Copilot auf einer GitHub Enterprise-Instanz
- `uvx gac auth copilot logout` — Von Copilot abmelden und gespeicherte Tokens entfernen
- `uvx gac auth copilot status` — Copilot-Authentifizierungsstatus prüfen
- `uvx gac config show` — Aktuelle Konfiguration anzeigen
- `uvx gac config set KEY VALUE` — Konfigurationsschlüssel in `$HOME/.gac.env` setzen
- `uvx gac config get KEY` — Konfigurationswert abrufen
- `uvx gac config unset KEY` — Konfigurationsschlüssel aus `$HOME/.gac.env` entfernen
- `uvx gac language` (oder `uvx gac lang`) — Interaktiver Sprachselektor für Commit-Nachrichten (setzt GAC_LANGUAGE)
- `uvx gac editor` (oder `uvx gac edit`) — Interaktiver Editor-Selektor für die `e`-Taste am Bestätigungsprompt (setzt GAC_EDITOR)
- `uvx gac diff` — Gefiltertes git diff mit Optionen für gestufte/ungestufte Änderungen, Farbe und Kürzung anzeigen
- `uvx gac serve` — GAC als [MCP-Server](MCP.md) für KI-Agenten-Integration starten (stdio-Transport)
- `uvx gac stats show` — Ihre gac-Nutzungsstatistiken anzeigen (Summen, Streaks, tägliche & wöchentliche Aktivität, Token-Nutzung, Top-Projekte mit Ø Dateien pro Gac, Top-Modelle mit Geschwindigkeit und Latenz)
- `uvx gac stats models` — Detaillierte Statistiken für alle Modelle mit Token-Aufschlüsselung, Geschwindigkeit, Latenz und Latenz-pro-Commit-Vergleichsdiagrammen
- `uvx gac stats projects` — Statistiken für alle Projekte mit Token-Aufschlüsselung und Ø Dateien pro Gac
- `uvx gac stats recent` — Letzte 10 Gacs mit Tokens, Geschwindigkeit, Latenz und Dateien pro Gac (`-n 20` für mehr)
- `uvx gac stats reset` — Alle Statistiken auf Null zurücksetzen (mit Bestätigungsaufforderung)
- `uvx gac stats reset model <model-id>` — Statistiken für ein bestimmtes Modell zurücksetzen (Groß-/Kleinschreibung wird nicht berücksichtigt)
...
```sh
uvx gac -i -h "Datenbankmigration für Benutzerprofile"
...
uvx gac erfasst leichtgewichtige Nutzungsstatistiken, damit Sie Ihre Commit-Aktivität, Streaks, Token-Nutzung und aktivsten Projekte und Modelle sehen können. Statistiken werden lokal in `~/.gac_stats.json` gespeichert und niemals versendet — es gibt keine Telemetrie.
...
`uvx gac init` fragt, ob Sie Statistiken aktivieren möchten, und erklärt genau, was gespeichert wird. Sie können Ihre Meinung jederzeit ändern:
...
Wenn Sie Statistiken während `uvx gac init` ablehnen und eine vorhandene `~/.gac_stats.json` erkannt wird, wird Ihnen die Möglichkeit angeboten, diese zu löschen.

### Statistik-Unterbefehle

| Befehl                                 | Beschreibung                                                                                                                                  |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `uvx gac stats`                        | Ihre Statistiken anzeigen (gleich wie `uvx gac stats show`)                                                                                   |
| `uvx gac stats show`                   | Vollständige Statistiken anzeigen: Summen, Streaks, tägliche & wöchentliche Aktivität, Token-Nutzung, Top-Projekte, Top-Modelle               |
| `uvx gac stats models`                 | Detaillierte Statistiken für **alle** verwendeten Modelle mit Token-Aufschlüsselung, Geschwindigkeit, Latenz und Latenz-pro-Commit-Diagrammen |
| `uvx gac stats projects`               | Statistiken für **alle** Projekte mit Token-Aufschlüsselung und Ø Dateien pro Gac                                                             |
| `uvx gac stats recent`                 | Letzte 10 Gacs (`-n 20` für mehr), mit Tokens, Geschwindigkeit, Latenz und Dateien pro Gac                                                    |
| `uvx gac stats reset`                  | Alle Statistiken auf Null zurücksetzen (mit Bestätigungsaufforderung)                                                                         |
| `uvx gac stats reset model <model-id>` | Statistiken für ein bestimmtes Modell zurücksetzen (Groß-/Kleinschreibung wird nicht berücksichtigt)                                          |
...
Die Ausführung von `uvx gac stats` zeigt:
...
- **Top-Projekte** — Ihre 5 aktivsten Repos nach gac- + Commit-Anzahl, mit Ø Dateien pro Gac und Token-Nutzung
- **Top-Modelle** — Ihre 5 am häufigsten verwendeten Modelle mit Gesamtgeschwindigkeit, Latenz und Token-Nutzung

Die Ausführung von `uvx gac stats projects` zeigt **alle** Projekte (nicht nur die Top 5) mit:

- **Alle Projekte-Tabelle** — jedes Projekt sortiert nach Aktivität, mit Gac-Anzahl, Commit-Anzahl, Commits-pro-Gac-Verhältnis, Ø Dateien pro Gac, Prompt-Token, Output-Token, Reasoning-Token, Gesamt-Token und Anteil an Gesamt-Gacs
- **Aktivitätsbalkendiagramm** — horizontale Balken mit relativer Gac-Anzahl pro Projekt
- **Token-Nutzungsbalkendiagramm** — horizontale Balken mit relativem Token-Verbrauch pro Projekt

Die Ausführung von `uvx gac stats models` zeigt **alle** Modelle (nicht nur die Top 5) mit:

- **Alle Modelle-Tabelle** — jedes verwendete Modell sortiert nach Aktivität, mit Gac-Anzahl, Commit-Anzahl, Gesamtgeschwindigkeit (Tokens/Sek), Gesamtlatenz, Prompt-Token, Output-Token, Reasoning-Token und Gesamt-Token
- **Geschwindigkeitsvergleich (30d)-Diagramm** — ein horizontales Balkendiagramm der letzten (30 Tage) Modellgeschwindigkeiten, sortiert von schnellstem zu langsamstem, farbcodiert nach Geschwindigkeitsperzentil (🟡 blitzschnell, 🟢 schnell, 🔵 moderat, 🔘 langsam)
- **Latenzvergleich (30d)-Diagramm** — ein horizontales Balkendiagramm der letzten Latenz pro Aufruf, sortiert von kürzester zu längster
- **Latenz-pro-Commit (30d)-Diagramm** — ein horizontales Balkendiagramm der letzten Latenz geteilt durch Commit-Anzahl, zeigt die echte Wartezeit pro Commit (ein Modell, das 5 Commits in einem 10s-Gac macht, kostet 2s/Commit vs. eines, das 1 Commit in einem 25s-Gac bei 25s/Commit macht)
...
Die Ausführung von `uvx gac stats recent` zeigt Ihre letzten 10 Gacs (konfigurierbar mit `-n`):

- **Letzte Gacs-Tabelle** — jeder Gac mit relativer Zeit, Projekt, Modell, Commit-Anzahl, Dateien, Geschwindigkeit, Latenz und Token-Aufschlüsselung pro Gac
...
gac kann einen Discord-Kanal jedes Mal anpingen, wenn du einen Commit landest, unter Verwendung einer Webhook-URL aus den Integrationseinstellungen deines Kanals. Die Integration ist **opt-in**: Sie tut nichts, bis du explizit eine Webhook-URL konfigurierst.

### Einrichtung

Verwende die dedizierte `discord`-Subcommand-Gruppe:

```bash
uvx gac discord setup     # interaktive Konfiguration einer Webhook-URL
uvx gac discord show      # zeigt an, ob ein Webhook konfiguriert ist (URL maskiert)
uvx gac discord test      # sendet eine Testbenachrichtigung an den konfigurierten Webhook
uvx gac discord remove    # entfernt die konfigurierte Webhook-URL
```

Alternativ kannst du die Variable direkt in `$HOME/.gac.env` (oder `./.gac.env`) setzen:
...
- Feuert nach jedem erfolgreichen Commit (einzelne und gruppierte Workflows). Wird bei `--dry-run` und `--message-only` übersprungen.
- Postet ein **Embed** im GitHub-Stil mit einem grünen Streifen, Repo + Branch als Autorenzeile, der Commit-Betreff als Titel, der Commit-Body als Beschreibung und der kurze SHA im Footer.
- Verwendet den gac-Avatar und den Benutzernamen `gac`.
- Webhook-Fehler werden auf WARNING geloggt und blockieren **niemals** deinen Commit.
- Lass `GAC_DISCORD_WEBHOOK_URL` ungesetzt (oder leer) um zu deaktivieren. `gac init` ist davon nicht betroffen — Discord-Einrichtung lebt nur unter `gac discord`.
