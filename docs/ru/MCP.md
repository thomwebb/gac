...
MCP-сервер запускается командой `gac serve` и общается через stdio - стандартный транспорт MCP.
...
1. **Провайдер и модель**: Запустите `gac init` или `gac model` для настройки вашего ИИ-провайдера
2. **API-ключи**: Хранятся в `~/.gac.env` (настраиваются при `gac init`)
...
Запустите `gac init` для настройки вашего ИИ-провайдера и модели перед использованием MCP-сервера.
...
MCP-сервер автоматически перенаправляет весь вывод Rich-консоли в stderr для предотвращения повреждения протокола stdio. Если вы видите искажённый вывод, убедитесь, что запускаете `gac serve` (а не `gac` напрямую) при использовании MCP.
