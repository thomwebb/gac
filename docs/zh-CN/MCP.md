...
MCP 服务器通过 `uvx gac serve` 启动，并通过 stdio（标准 MCP 传输）进行通信。
...
1. **提供商和模型**：运行 `uvx gac init` 或 `uvx gac model` 配置你的 AI 提供商
2. **API 密钥**：存储在 `~/.gac.env` 中（在 `uvx gac init` 期间设置）
...
在使用 MCP 服务器之前，运行 `uvx gac init` 设置你的 AI 提供商和模型。
...
MCP 服务器会自动将所有 Rich 控制台输出重定向到 stderr，以防止 stdio 协议损坏。如果你看到乱码输出，请确保在使用 MCP 时运行 `uvx gac serve`（而非直接运行 `uvx gac`）。
