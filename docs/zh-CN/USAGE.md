...
uvx gac 包含内置的安全扫描，在提交之前自动检测暂存更改中的潜在密钥和 API 密钥。这有助于防止意外提交敏感信息。
...
- `uvx gac stats show` — 查看你的 gac 使用统计（总计、连续使用、每日和每周活动、令牌使用量、热门项目、热门模型）
- `uvx gac stats models` — 查看所有模型的详细统计，包含令牌明细和速度对比图表
- `uvx gac stats projects` — 查看所有项目的统计，包含令牌明细
...
| `uvx gac stats models`                 | 显示**所有**已使用模型的详细统计，包含令牌明细和速度对比图表                 |
| `uvx gac stats projects`               | 显示**所有**项目的统计，包含令牌明细                                         |
| `uvx gac stats reset`                  | 重置所有统计数据为零（需要确认）                                             |
| `uvx gac stats reset model <model-id>` | 重置特定模型的统计数据（不区分大小写）                                       |
...
Running `uvx gac stats projects` 显示**所有**项目（不仅仅是前 5 个），包含：

- **所有项目表格** — 每个项目按活动排序，包含 gac 次数、提交次数、提示令牌、完成令牌、推理令牌和总令牌
- **热门模型** — 5 个最常用模型及其提示、完成和总令牌消耗量

Running `uvx gac stats models` 显示**所有**模型（不仅仅是前 5 个），包含：
