...
本文档介绍了 `uvx gac` CLI 工具的所有可用标志和选项。
...
- 组合标志以获得更强大的工作流（例如，`uvx gac -ayp` 以暂存、自动确认和推送）
...
```sh
uvx gac --no-verify  # 跳过所有 pre-commit 和 lefthook 钩子
```
...
uvx gac 包含内置的安全扫描，在**任何 AI API 调用之前**自动检测暂存更改中的潜在密钥和 API 密钥。如果检测到密钥，工作流将立即中止 — 不会进行任何 API 调用。这确保了您的敏感数据永远不会发送到任何 AI 模型。扫描程序使用**基于正则表达式的模式匹配**，而非 LLM，因此扫描速度快且完全在本地运行。

**跳过安全扫描：**

```sh
uvx gac --skip-secret-scan  # 为此次提交跳过安全扫描
```
...
uvx gac 支持跳过 SSL 证书验证，适用于企业代理拦截 SSL 流量并导致证书验证失败的环境。

**跳过 SSL 验证：**

```sh
uvx gac --no-verify-ssl  # 为此次提交跳过 SSL 验证
```
...
uvx gac 支持向提交信息添加 `Signed-off-by` 行，这在许多开源项目中是 [Developer Certificate of Origin (DCO)](https://developercertificate.org/) 合规所必需的。

**添加 signoff :**

```sh
uvx gac --signoff  # 添加 Signed-off-by 行到提交信息（DCO 合规）
```
...
- 设置 gac 的推荐方法是运行 `uvx gac init` 并按照交互式提示操作。
- 已经配置好语言，只想切换提供商或模型？运行 `uvx gac model`，它会跳过所有语言相关的问题。
...
- `GAC_ALWAYS_GROUPED=true` - 始终使用分组提交模式（相当于始终传递 `-g` 或 `--group` 标志）
...
- `GAC_REASONING_EFFORT=medium` - 控制支持扩展思考模型的推理/思考深度（low、medium、high、max）。不设置则使用各模型默认值。仅发送至兼容的提供者（OpenAI 风格；非 Anthropic）。
...
- `uvx gac init` — 提供商、模型和语言配置的交互式设置向导
- `uvx gac model` — 提供商/模型/API 密钥设置，无语言提示（适合快速切换）
- `uvx gac auth` — 显示所有提供商的 OAuth 身份验证状态
- `uvx gac auth claude-code login` — 使用 OAuth 登录 Claude Code（打开浏览器）
- `uvx gac auth claude-code logout` — 从 Claude Code 注销并删除存储的令牌
- `uvx gac auth claude-code status` — 检查 Claude Code 身份验证状态
- `uvx gac auth chatgpt login` — 使用 OAuth 登录 ChatGPT（打开浏览器）
- `uvx gac auth chatgpt logout` — 从 ChatGPT 注销并删除存储的令牌
- `uvx gac auth chatgpt status` — 检查 ChatGPT 身份验证状态
- `uvx gac auth copilot login` — 使用 Device Flow 登录 GitHub Copilot
- `uvx gac auth copilot login --host ghe.mycompany.com` — 登录 GitHub Enterprise 实例上的 Copilot
- `uvx gac auth copilot logout` — 从 Copilot 注销并删除存储的令牌
- `uvx gac auth copilot status` — 检查 Copilot 身份验证状态
- `uvx gac config show` — 显示当前配置
- `uvx gac config set KEY VALUE` — 在 `$HOME/.gac.env` 中设置配置键
- `uvx gac config get KEY` — 获取配置值
- `uvx gac config unset KEY` — 从 `$HOME/.gac.env` 中删除配置键
- `uvx gac language`（或 `uvx gac lang`）— 提交消息的交互式语言选择器（设置 GAC_LANGUAGE）
- `uvx gac editor`（或 `uvx gac edit`）— 确认提示符中 `e` 键的交互式编辑器选择器（设置 GAC_EDITOR）
- `uvx gac diff` — 显示过滤的 git diff，具有暂存/未暂存更改、颜色和截断选项
- `uvx gac serve` — 将 GAC 作为 [MCP 服务器](MCP.md) 启动，用于 AI 代理集成（stdio 传输）
- `uvx gac stats show` — 查看你的 gac 使用统计（总计、连续使用、每日和每周活动、令牌使用量、带平均文件数的热门项目、带速度和延迟的热门模型）
- `uvx gac stats models` — 所有模型的详细统计，包含令牌细分、速度、延迟和每次提交延迟对比图表
- `uvx gac stats projects` — 所有项目的统计，包含令牌细分和每次 gac 平均文件数
- `uvx gac stats recent` — 最近 10 次 gac 的历史记录，包含令牌、速度、延迟、文件数（`-n 20` 查看更多）
- `uvx gac stats reset` — 重置所有统计数据为零（需要确认）
- `uvx gac stats reset model <model-id>` — 重置特定模型的统计数据（不区分大小写）
...
uvx gac 追踪轻量级使用统计，让你可以查看提交活动、连续使用天数、令牌使用量和最活跃的项目及模型。统计数据本地存储在 `~/.gac_stats.json` 中，不会发送到任何地方 — 无遥测。
...
`uvx gac init` 会询问是否启用统计，并详细说明存储的内容。你可以随时改变主意：

- **启用统计：** 取消设置 `GAC_DISABLE_STATS` 或设置为 `false`/`0`/`no`/`off`/空。
- **禁用统计：** 将 `GAC_DISABLE_STATS` 设置为 truthy 值（`true`、`1`、`yes`、`on`）。

在 `uvx gac init` 中拒绝统计时，如果检测到现有的 `~/.gac_stats.json`，将提供删除选项。

### 统计子命令

| 命令                                   | 描述                                                                         |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `uvx gac stats`                        | 显示你的统计（等同于 `uvx gac stats show`）                                  |
| `uvx gac stats show`                   | 显示完整统计：总计、连续使用、每日和每周活动、令牌使用量、热门项目、热门模型 |
| `uvx gac stats models`                 | 所有使用模型的详细统计，包含令牌细分、速度、延迟和每次提交延迟图表           |
| `uvx gac stats projects`               | 所有项目的统计，包含令牌细分和每次 gac 平均文件数                            |
| `uvx gac stats recent`                 | 最近 10 次 gac（`-n 20` 查看更多），包含令牌、速度、延迟、文件数             |
| `uvx gac stats reset`                  | 重置所有统计数据为零（需要确认）                                             |
| `uvx gac stats reset model <model-id>` | 重置特定模型的统计数据（不区分大小写）                                       |
...
运行 `uvx gac stats` 将显示：

- **gac 使用总次数和提交总次数** — 你使用 gac 的次数和它创建的提交数
- **当前和最长连续使用天数** — 连续使用 gac 的天数（5 天及以上显示 🔥）
- **活动摘要** — 今日和本周的 gac 使用、提交和令牌数与你的峰值日和峰值周对比
- **热门项目** — 按 gac 使用 + 提交数排列的 5 个最活跃仓库，包含每次 gac 平均文件数和令牌使用量
- **热门模型** — 5 个最常用模型及其全时段速度、延迟和令牌使用量

运行 `uvx gac stats projects` 显示**所有**项目（不仅仅是前 5 个），包含：

- **所有项目表格** — 每个项目按活动排序，包含 gac 次数、提交次数、每次 gac 提交比率、每次 gac 平均文件数、提示令牌、完成令牌、推理令牌、总令牌和总 gac 占比
- **活动条形图** — 水平条形显示每个项目的相对 gac 次数
- **令牌使用量条形图** — 水平条形显示每个项目的相对令牌消耗量

运行 `uvx gac stats models` 显示**所有**模型（不仅仅是前 5 个），包含：

- **所有模型表格** — 每个已使用的模型按活动排序，包含 gac 次数、提交次数、全时段速度（令牌/秒）、全时段延迟、提示令牌、完成令牌、推理令牌和总令牌
- **速度对比（30天）图表** — 最近（过去 30 天）模型速度的水平条形图，从最快到最慢排序，按速度百分位着色（🟡 极速、🟢 快速、🔵 中等、🔘 慢速）
- **延迟对比（30天）图表** — 最近每次调用延迟的水平条形图，从最短到最长排序
- **每次提交延迟（30天）图表** — 最近延迟除以提交次数的水平条形图，显示每次提交的实际等待时间（在 10 秒 gac 中完成 5 次提交的模型每次提交耗时 2 秒 vs 在 25 秒 gac 中完成 1 次提交的模型每次提交耗时 25 秒）
- **高分庆祝** — 🏆 在你创造新的每日、每周、令牌或连续使用记录时获得奖杯；🥈 在追平记录时获得
- **鼓励信息** — 基于你活动的上下文鼓励

运行 `uvx gac stats recent` 显示你最近 10 次 gac（可通过 `-n` 配置）：

- **最近 gac 表格** — 每次 gac 包含相对时间、项目、模型、提交次数、文件数、速度、延迟和每次 gac 的令牌明细
...
gac 可以在每次提交时通过您频道集成设置中的 Webhook URL 向 Discord 频道发送通知。此集成是**可选的**：在您明确配置 Webhook URL 之前，它不会执行任何操作。
...
```bash
uvx gac discord setup     # 以交互方式配置 Webhook URL
uvx gac discord show      # 显示是否已配置 Webhook（URL 已屏蔽）
uvx gac discord test      # 向已配置的 Webhook 发送测试通知
uvx gac discord remove    # 删除已配置的 Webhook URL
```

或者，直接在 `$HOME/.gac.env`（或 `./.gac.env`）中设置变量：
...
- 在每次成功提交后触发（单个和分组的工作流）。在 `--dry-run` 和 `--message-only` 时跳过。
- 发布带有绿色条纹的 GitHub 风格 **嵌入**，仓库 + 分支作为作者行，提交主题作为标题，提交正文作为描述，短 SHA 在页脚中。
- 使用 gac 头像和用户名 `gac`。
- Webhook 失败会以 WARNING 级别记录，并且**从不**阻止您的提交。
- 将 `GAC_DISCORD_WEBHOOK_URL` 保持未设置（或为空）以禁用。`gac init` 不受影响 — Discord 设置仅位于 `gac discord` 下。
