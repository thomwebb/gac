...
- 按照 [astral.sh/uv](https://astral.sh/uv) 上的说明安装 uv
...
- 如果你是新用户，运行 `uvx gac init` 以交互方式设置你的提供商、模型和 API 密钥
- 确保你的 `.gac.env` 或环境变量设置正确
- 运行 `uvx gac --log-level=debug` 查看加载了哪些配置文件并调试配置问题
...
- 运行 `uvx gac --log-level=debug` 确认加载了用户级配置
...
- 运行 `uvx gac --log-level=debug` 确认加载了项目级配置
- 如果你编辑了 `.gac.env`，重启你的终端或重新运行你的 shell 以重新加载环境变量
- 如果仍然无效，检查拼写错误和文件权限

**问题：** 无法设置或更改提交信息的语言

- 运行 `uvx gac language`（或 `uvx gac lang`）以交互方式从 25+ 种支持的语言中选择
- 使用 `-l <语言>` 标志为单次提交覆盖语言（例如，`uvx gac -l zh-CN`，`uvx gac -l Spanish`）
- 使用 `uvx gac config show` 检查你的配置以查看当前语言设置
...
- **对于Claude Code令牌过期**：运行 `uvx gac auth` 以快速重新身份验证并刷新令牌。浏览器将自动打开进行OAuth。
- **对于其他Claude Code OAuth问题**，请参阅[Claude Code设置指南](CLAUDE_CODE.md)获取全面的故障排除。
- **对于 GitHub Copilot 会话令牌过期**：运行 `uvx gac auth copilot login` 通过 Device Flow 重新认证。会话令牌会从缓存的 OAuth 令牌自动刷新。
...
- 使用 `uvx gac --show-prompt` 调试 LLM 看到的内容
...
**重要：**密钥扫描在**任何 AI API 调用之前**运行。如果检测到密钥，工作流将立即中止，不会进行任何 API 调用。扫描程序使用**基于正则表达式的模式匹配**（而非 LLM），因此扫描速度快且完全在本地运行 — 您的代码永远不会发送到 AI 模型进行密钥检测。

**问题：** 误报：密钥扫描检测到非密钥

- 安全扫描程序会查找类似于 API 密钥、令牌和密码的正则表达式模式
...
- 扫描程序使用基于正则表达式的模式匹配（而非 LLM），可能无法捕获所有密钥类型
...
- 使用 `uvx gac --no-verify` 暂时跳过所有 pre-commit 和 lefthook 钩子
- 修复导致钩子失败的根本问题
...
- gac 需要暂存的更改来生成提交信息
- 使用 `git add <文件>` 暂存更改，或使用 `uvx gac -a` 自动暂存所有更改
- 检查 `git status` 查看哪些文件已被修改
- 使用 `uvx gac diff` 查看你的更改的过滤视图
...
- 使用 `uvx gac -y` 跳过确认提示
- 使用 `uvx gac -q` 以静默模式减少输出
- 考虑为常规提交使用更快/更便宜的模型
- 如果钩子让你慢下来，使用 `uvx gac --no-verify` 跳过钩子
...
- 使用 `uvx gac init` 以交互方式重置或更新你的配置
- 使用 `uvx gac --log-level=debug` 获取详细的调试输出和日志记录
- 使用 `uvx gac --show-prompt` 查看发送给 LLM 的提示
- 使用 `uvx gac --help` 查看所有可用的命令行标志
- 使用 `uvx gac config show` 查看所有当前配置值
- 检查日志以查找错误消息和堆栈跟踪
