...
Tài liệu này mô tả tất cả các flag và tùy chọn có sẵn cho công cụ CLI `uvx gac`.
...
- **Thêm gợi ý cho LLM:**
  ...
  ```sh
  uvx gac -h "Tái cấu trúc logic xác thực"
  ```
...
- **Tạo thông điệp commit bằng ngôn ngữ cụ thể:**
  ...
  # Sử dụng tên đầy đủ
  uvx gac -l "Tiếng Trung Đơn Giản"
  uvx gac -l Japanese
  uvx gac -l Spanish
  ```
...
- Kết hợp các flag để có các quy trình làm việc mạnh mẽ hơn (ví dụ, `uvx gac -ayp` để stage, tự động xác nhận, và push)
...
```sh
uvx gac --no-verify  # Bỏ qua tất cả các hook pre-commit và lefthook
```
...
```sh
uvx gac --skip-secret-scan  # Bỏ qua quét bảo mật cho commit này
```
...
```sh
uvx gac --no-verify-ssl  # Bỏ qua xác minh SSL cho commit này
```
...
uvx gac hỗ trợ thêm dòng `Signed-off-by` vào thông điệp commit, được yêu cầu cho việc tuân thủ [Developer Certificate of Origin (DCO)](https://developercertificate.org/) trong nhiều dự án mã nguồn mở.

**Thêm signoff :**

```sh
uvx gac --signoff  # Thêm dòng Signed-off-by vào thông điệp commit (tuân thủ DCO)
```
...
- Cách được đề xuất để thiết lập gac là chạy `uvx gac init` và làm theo các gợi ý tương tác.
- Đã cấu hình ngôn ngữ và chỉ cần chuyển đổi nhà cung cấp hoặc mô hình? Chạy `uvx gac model` để lặp lại thiết lập mà không có câu hỏi ngôn ngữ.
...
- `GAC_REASONING_EFFORT=medium` - Kiểm soát độ sâu lập luận/suy nghĩ cho các mô hình hỗ trợ suy nghĩ mở rộng (low, medium, high, max). Để trống để sử dụng mặc định của từng mô hình. Chỉ gửi đến các nhà cung cấp tương thích (kiểu OpenAI; không phải Anthropic).
...
- `uvx gac init` — Trình hướng dẫn thiết lập tương tác cho nhà cung cấp, mô hình và cấu hình ngôn ngữ
- `uvx gac model` — Thiết lập nhà cung cấp/mô hình/khoá API không có lời nhắc ngôn ngữ (lý tưởng cho các thay đổi nhanh)
- `uvx gac auth` — Hiển thị trạng thái xác thực OAuth cho tất cả nhà cung cấp
- `uvx gac auth claude-code login` — Đăng nhập vào Claude Code sử dụng OAuth (mở trình duyệt)
- `uvx gac auth claude-code logout` — Đăng xuất khỏi Claude Code và xóa token đã lưu
- `uvx gac auth claude-code status` — Kiểm tra trạng thái xác thực Claude Code
- `uvx gac auth chatgpt login` — Đăng nhập vào ChatGPT sử dụng OAuth (mở trình duyệt)
- `uvx gac auth chatgpt logout` — Đăng xuất khỏi ChatGPT và xóa token đã lưu
- `uvx gac auth chatgpt status` — Kiểm tra trạng thái xác thực ChatGPT OAuth
- `uvx gac auth copilot login` — Đăng nhập GitHub Copilot bằng Device Flow
- `uvx gac auth copilot login --host ghe.mycompany.com` — Đăng nhập Copilot trên phiên bản GitHub Enterprise
- `uvx gac auth copilot logout` — Đăng xuất Copilot và xóa token đã lưu
- `uvx gac auth copilot status` — Kiểm tra trạng thái xác thực Copilot
- `uvx gac config show` — Hiển thị cấu hình hiện tại
- `uvx gac config set KEY VALUE` — Đặt khóa cấu hình trong `$HOME/.gac.env`
- `uvx gac config get KEY` — Lấy giá trị cấu hình
- `uvx gac config unset KEY` — Xóa khóa cấu hình khỏi `$HOME/.gac.env`
- `uvx gac language` (hoặc `uvx gac lang`) — Trình chọn ngôn ngữ tương tác cho các thông điệp commit (đặt GAC_LANGUAGE)
...
```sh
uvx gac -i -h "Di chuyển cơ sở dữ liệu cho hồ sơ người dùng"
...
uvx gac theo dõi thống kê sử dụng nhẹ để bạn có thể xem hoạt động commit, chuỗi, sử dụng token, và các dự án và mô hình tích cực nhất của mình. Thống kê được lưu trữ cục bộ trong `~/.gac_stats.json` và không bao giờ được gửi đi đâu — không có thu thập dữ liệu từ xa.
...
`uvx gac init` hỏi bạn có muốn bật thống kê và giải thích chính xác những gì được lưu trữ. Bạn có thể thay đổi ý kiến bất cứ lúc nào:
...
Khi bạn từ chối thống kê trong `uvx gac init` và phát hiện tệp `~/.gac_stats.json` hiện có, bạn sẽ được đề xuất tùy chọn xóa nó.
...
gac có thể ping một kênh Discord mỗi khi bạn thực hiện một commit, sử dụng URL webhook từ cài đặt tích hợp của kênh của bạn. Tích hợp này là **opt-in**: nó không làm gì cho đến khi bạn cấu hình rõ ràng một URL webhook.

### Thiết lập

Sử dụng nhóm lệnh con `discord` chuyên dụng:

```bash
uvx gac discord setup     # cấu hình URL webhook theo cách tương tác
uvx gac discord show      # hiển thị xem có webhook nào được cấu hình hay không (URL được che)
uvx gac discord test      # gửi thông báo kiểm tra đến webhook đã cấu hình
uvx gac discord remove    # xóa URL webhook đã cấu hình
```

Ngoài ra, bạn có thể đặt biến trực tiếp trong `$HOME/.gac.env`（hoặc`./.gac.env`):
...
- Kích hoạt sau mỗi commit thành công（workflow đơn lẻ và được nhóm）。Bỏ qua khi `--dry-run` và `--message-only`。
- Đăng một **embed** kiểu GitHub với một dải màu xanh, repo + nhánh làm hàng tác giả, chủ đề commit làm tiêu đề, nội dung commit làm mô tả và SHA ngắn ở chân trang。
- Sử dụng avatar gac và tên người dùng `gac`。
- Lỗi webhook được ghi log ở mức WARNING và **không bao giờ** chặn commit của bạn。
- Để `GAC_DISCORD_WEBHOOK_URL` không được đặt（hoặc trống）để tắt。`gac init` không bị ảnh hưởng — thiết lập Discord chỉ sống dưới `gac discord`。
