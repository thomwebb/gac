...
```bash
gac stats               # 概要：合計 gac 数、ストリーク、毎日/毎週のピーク、トッププロジェクトとモデル
gac stats models        # モデル別内訳：gac 数、トークン、レイテンシ、速度
gac stats projects      # プロジェクト別内訳：全リポジトリの gac 数、コミット数、トークン数
gac stats reset         # 全統計をリセット（確認あり）
gac stats reset model <model-id>  # 特定モデルの統計のみリセット
```
...
**別の言語でコミットメッセージが必要ですか？** `gac language を実行して、Español、Français、日本語など 25+ の言語から選択してください。
