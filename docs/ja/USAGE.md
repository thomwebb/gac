...
uvx gac には組み込みセキュリティスキャンが含まれており、コミット前にステージされた変更で潜在的な秘密と API キーを自動的に検出します。これにより、機密情報の偶然のコミットを防ぎます。
...
- `uvx gac stats show` — gac の使用統計を表示（合計、ストリーク、日次・週次アクティビティ、トークン使用量、トッププロジェクト、トップモデル）
- `uvx gac stats models` — すべてのモデルのトークン内訳と速度比較チャート付きの詳細統計を表示
- `uvx gac stats projects` — すべてのプロジェクトのトークン内訳付き統計を表示
...
| `uvx gac stats models`                 | 使用した**すべて**のモデルのトークン内訳と速度比較チャート付きの詳細統計を表示                                 |
| `uvx gac stats projects`               | **すべて**のプロジェクトのトークン内訳付き統計を表示                                                           |
| `uvx gac stats reset`                  | すべての統計をゼロにリセット（確認プロンプト付き）                                                             |
| `uvx gac stats reset model <model-id>` | 特定のモデルの統計をリセット（大文字小文字を区別しない）                                                       |
...
Running `uvx gac stats projects` は（上位5つだけでなく）**すべて**のプロジェクトを次の内容で表示します：

- **全プロジェクトテーブル** — アクティビティ順の各プロジェクト、gac数、コミット数、プロンプトトークン、完了トークン、推論トークン、合計トークン付き
- **トップモデル** — プロンプト、完了、合計トークン消費量付きの上位5つの最も使用されているモデル

Running `uvx gac stats models` は（上位5つだけでなく）**すべて**のモデルを次の内容で表示します：
