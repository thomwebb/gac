...
- [astral.sh/uv](https://astral.sh/uv) の手順に従って uv をインストール
...
- 新しい場合、`uvx gac init` を実行して対話的にプロバイダー、モデル、APIキーを設定
- `.gac.env` または環境変数が正しく設定されていることを確認
- `uvx gac --log-level=debug` を実行してどの設定ファイルが読み込まれているかを確認し、設定問題をデバッグ
...
- `uvx gac --log-level=debug` を実行してユーザーレベル設定が読み込まれていることを確認
...
- `uvx gac --log-level=debug` を実行してプロジェクトレベル設定が読み込まれていることを確認
- `.gac.env` を編集した場合、環境変数をリロードするためにターミナルを再起動するか、シェルを再実行
- まだ動作しない場合、タイプミスとファイル権限を確認

**問題:** コミットメッセージの言語を設定または変更できない

- `uvx gac language`（または `uvx gac lang`）を実行して25以上の対応言語から対話的に選択
- `-l <language>` フラグを使用して単一コミットの言語を上書き（例: `uvx gac -l zh-CN`, `uvx gac -l Japanese`）
- `uvx gac config show` で現在の言語設定を確認
...
- **Claude Codeのトークン有効期限の場合**：`uvx gac auth`を実行して素早く再認証してトークンを更新します。ブラウザが自動的にOAuthのために開きます。
- **ChatGPT OAuth トークン有効期限の場合**：`uvx gac auth chatgpt login`を実行して再認証します。ブラウザが自動的に OAuth のために開きます。
...
- **GitHub Copilot セッショントークンの有効期限の場合**：Device Flow で再認証するには `uvx gac auth copilot login` を実行します。セッショントークンはキャッシュされた OAuth トークンから自動的に更新されます。
...
- `uvx gac --show-prompt` を使用してLLMが見ているものをデバッグ
...
**重要:** 秘密スキャンは**AI API呼び出しが行われる前に**実行されます。秘密が検出された場合、ワークフローは直ちに中止され、API呼び出しは行われません。スキャナーは**正規表現ベースのパターンマッチング**（LLMではない）を使用するため、スキャンは高速で完全にローカルで実行されます — 秘密検出のためにコードがAIモデルに送信されることはありません。

**問題:** 偽陽性: 秘密スキャンが非秘密を検出

- セキュリティスキャナーはAPIキー、トークン、パスワードに似た正規表現パターンを探します
...
- スキャナーは正規表現ベースのパターンマッチング（LLMではない）を使用し、すべての秘密タイプをキャッチできない場合があります
...
- `uvx gac config set GAC_SKIP_SECRET_SCAN true` を使用
...
- `uvx gac --no-verify` を使用して一時的にすべてのpre-commitとlefthookフックをスキップ
- フックが失敗している根本的な問題を修正
...
- `git add <files>` で変更をステージするか、`uvx gac -a` ですべての変更を自動的にステージ
- `git status` を使用して変更されたファイルを確認
- `uvx gac diff` を使用して変更のフィルタリングされたビューを確認
...
- `uvx gac -y` を使用して確認プロンプトをスキップ
- `uvx gac -q` を使用して出力が少ない静默モードを使用
- 定期的なコミットにより高速/安価なモデルを検討
- フックが速度を落としている場合、`uvx gac --no-verify` を使用してフックをスキップ
...
- `uvx gac init` を使用して設定を対話的にリセットまたは更新
- `uvx gac --log-level=debug` を使用して詳細なデバッグ出力とログを取得
- `uvx gac --show-prompt` を使用してLLMに送信されるプロンプトを確認
- `uvx gac --help` を使用して利用可能なすべてのコマンドラインフラグを確認
- `uvx gac config show` を使用して現在のすべての設定値を確認
