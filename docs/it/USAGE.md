...
- `GAC_REASONING_EFFORT=medium` - Controllare la profondità di ragionamento/pensiero per modelli che supportano il pensiero esteso (low, medium, high, max). Lasciare non impostato per usare il predefinito di ciascun modello. Inviato solo a provider compatibili (stile OpenAI; non Anthropic).
