...
- `GAC_REASONING_EFFORT=medium` - Controlar la profundidad de razonamiento/pensamiento para modelos que soportan pensamiento extendido (low, medium, high, max). Dejar sin configurar para usar el valor predeterminado de cada modelo. Solo enviado a proveedores compatibles (estilo OpenAI; no Anthropic).
